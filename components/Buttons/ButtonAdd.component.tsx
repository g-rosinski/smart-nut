...
interface ButtonAddProps extends Omit<ButtonProps, 'title'>{
    style?: ViewStyle
    title?: string
}

const ButtonAdd: React.FC<ButtonAddProps> = ({disabled, style, title, ...props}) => (
    <TouchableOpacity style={[styles.button, style, disabled? styles.disabled : null]} disabled={disabled} {...props}>
        <Ionicons name="md-add" size={26} color={ colors.copper } style={styles.icon} />
